...
**接口**：POST /api/tools/{toolId}/offline
...
\\json
{
  ...
  "data": {
    ...
    "status": "offline",
    "offline_at": "2026-09-08T12:00:00Z"
  }
}
\\n
=======
...
**接口**：POST /api/servers/{serverId}/activate
...
{
  "code": 0,
  "message": "MCP Server 已激活",
  "data": {
    "server_id": "srv-demo-db",
    "status": "published",
    "activated_at": "2026-09-08T12:00:00Z"
  }
}
\\n
...
**接口**：POST /api/servers/{serverId}/offline
...
**接口**：DELETE /api/tools/{toolId}/permissions/{roleId}
...
**接口**：GET /api/tools/{toolId}/permissions/check

**权限**：无需额外认证（使用当前请求的 JWT 身份）
...
| 参数 | 类型 | 必填 | 说明 |
|---|---|---|---|
| action | string | 是 | 操作类型：read / call |
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| event_type | string | 是 | 事件类型：login/call/permission_grant/permission_revoke/tool_publish/tool_unpublish/server_register/server_activate/server_offline |
| resource_type | string | 是 | 资源类型：user/tool/server/permission |
| resource_id | string | 是 | 资源 ID |
| operator_id | string | 否 | 操作者 ID |
...
\\json
{
  "event_type": "call",
  "resource_type": "tool",
  "resource_id": "tool-mysql-query",
  "operator_id": "usr-agent-01",
  "detail": "调用 mysql_query 工具成功",
